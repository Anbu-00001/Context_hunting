...
```mermaid
graph TD
    Root["📁 Context_hunting/"] --> Public["📁 public/"]
    Root --> Src["📁 src/"]
    Root --> Docs["📁 docs/"]
    Root --> N8N_Dir["📁 n8n/"]
    Root --> Tools["📁 tools/"]
    ...
    Public --> PanelHTML["📄 context-panel.html\n+ context-panel.css"]
    ...
    Config --> Devvit["📄 devvit.json"]
    Config --> Env["📄 .env.example"]
    Config --> Pkg["📄 package.json"]

    style Root fill:#cc3700,stroke:#ff6633,color:#fff
    style Public fill:#2563eb,stroke:#5b86e5,color:#fff
    ...
    style Tools fill:#7f8c8d,stroke:#95a5a6,color:#fff
    style Config fill:#4a6741,stroke:#6b8f63,color:#fff
    style PanelHTML fill:#2874a6,stroke:#3498db,color:#fff
    style DashHTML fill:#2874a6,stroke:#3498db,color:#fff
    style Client fill:#0e7c61,stroke:#2ecc71,color:#fff
    style Server fill:#8e44ad,stroke:#a569bd,color:#fff
    style Shared fill:#5b86e5,stroke:#7ea1ed,color:#fff
    ...
    style Devvit fill:#c0392b,stroke:#e74c3c,color:#fff
...
```
Context_hunting/
├── public/                       # WebView HTML & CSS
│   ├── context-panel.html        # Mod context panel UI
│   ├── context-panel.css
│   ├── dashboard.html            # QUORUM dashboard UI
│   └── dashboard.css
├── src/
│   ├── client/                   # Client-side TypeScript
│   │   ├── context-panel.ts      # Context panel logic
│   │   └── dashboard.ts          # Dashboard logic
│   ├── server/                   # Devvit server-side
│   │   ├── index.ts              # Server entry point
│   │   ├── server.ts             # Route handlers
│   │   └── engines/              # Core QUORUM engines
│   └── shared/                   # Shared types & API contracts
│       └── api.ts                # Client ↔ Server API definitions
├── tools/                        # Build tooling
